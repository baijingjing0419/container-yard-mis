  ...
  <div class="fade-in">
    ...
    <BaseModal title="新增调箱指令" :visible="showModal" @close="showModal=false" @save="handleSave">
      ...
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:15px;">
        ...
        <div class="form-group"><label class="form-label">目标堆位</label><input v-model="form.target_slot_id" class="form-input" placeholder="目标堆位"></div>
      </div>
...
async function handleSave() { if(!form.container_id)return alert('请输入箱号'); try{await createOperation({...form});showModal.value=false;alert('新增成功');fetchData()}catch(_){} }
onMounted(fetchData)
</script>
